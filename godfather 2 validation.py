...
# --------------------------------------------------------------------------
# FIX (see chat): shared helpers for custom_query fetches, used by BOTH the
# validate path (_validate_custom_query_batch) and the insert path
# (_insert_custom_query_batch / _load_custom_query_side). Previously each
# had its own copy; the insert path's copy never got the destination
# key-filtering fix, so it kept pulling the ENTIRE destination table
# (20M+ rows) on every run even after validate was fixed. Having exactly
# one implementation means fixing it once fixes it everywhere, and there is
# no config change needed -- both paths just filter whatever
# destination_fetch_query already says down to the keys actually in use.
# --------------------------------------------------------------------------

def fetch_custom_query_side(conn, side, sql, ref_cols, client_id, runtime_cfg, label=None, progress_cb=None):
    """Run a source/destination custom fetch query as-is (no key filtering),
    with caching. This is the ORIGINAL unfiltered behavior -- still used
    for the source side always (we need all of source), and as the
    fallback for destination when key-filtering isn't applicable (e.g. a
    composite join key)."""
    label = label or side
    ...
    rows, _ = execute_custom_fetch_query(conn, sql, ref_cols, runtime_cfg, progress_cb=progress_cb)
...
def fetch_destination_filtered_by_keys(conn, sql, ref_dest_cols, source_keys, client_id, flow_id,
    ...
    """
    FIX (see chat): destination_fetch_query, left exactly as configured, used
    to pull the WHOLE destination table (20M+ rows) unconditionally, no
    matter how few keys we actually needed (often a few thousand from
    source). This scopes the fetch down to exactly the keys source actually
    returned, via a #temp table + indexed JOIN -- more reliable at this
    scale than a giant literal IN (...) list, since SQL Server's optimizer
    can silently fall back to a full scan once an IN list gets large, while
    a real indexed join is much harder for it to get wrong. No hardcoded
    table/column names here -- dest_col, sql, and runtime_cfg options all
    come from config, so this works for whatever destination_fetch_query
    and reference_keys a given job defines.

    Falls back to the original unfiltered fetch only when:
      - source_keys is empty (nothing to compare -- skips destination
        entirely rather than pulling anything), or
      - the join key is composite (more than one reference key column --
        not special-cased here, so it warns and falls back rather than
        silently returning nothing).
    """
    cache_sql = sql + f"::keyfiltered::client={client_id}::flow={flow_id}{cache_label_suffix}"
    ...
    if not source_keys:
        log.info(f"[client={client_id}] Source returned 0 keys; skipping destination fetch entirely")
        return {}

    if len(ref_dest_cols) != 1:
        log.warning(
            f"[client={client_id}] Destination join key is composite ({len(ref_dest_cols)} columns); "
            "key-filtering only supports a single join key column, falling back to a full "
            "unfiltered destination fetch for this flow."
        )
        return fetch_custom_query_side(conn, "destination", sql, ref_dest_cols, client_id, runtime_cfg,
                                        label="destination")
    ...
    # FIX (see chat): keys are often Decimal/int from SQL Server, not plain
    # Python int. Round-tripping them through str() for an NVARCHAR temp
    # column can silently mangle them (e.g. Decimal('12345.00') -> "12345.00"
    # instead of "12345"), which then fails to match against an INT/BIGINT
    # destination column via implicit conversion -- rows that DO exist in
    # destination end up looking "missing" in the log. When every key is a
    # whole number, use a real BIGINT temp column with numeric values
    # inserted directly instead: no string formatting involved on either
    # side, and no CAST needed on the destination column either (keeping
    # its index usable). This is driven entirely by the actual key values
    # at runtime, not by the cast_temp_keys_for_join/temp_key_column_type
    # config options, which stay as the fallback for non-numeric keys.
    ...
    non_null_values = [v for v in flat_values if v is not None]
    use_numeric_key = bool(non_null_values) and all(_is_whole_number(v) for v in non_null_values)

    if use_numeric_key:
        col_type = "BIGINT"
        insert_rows = [(int(v) if v is not None else None,) for v in flat_values]
        join_col = f"base.{dest_col}"  # numeric-to-numeric, no cast needed on either side
    else:
        col_type = runtime_cfg.option("temp_key_column_type") or "NVARCHAR(500)"
        cast_for_join = runtime_cfg.option("cast_temp_keys_for_join")
        insert_rows = [(str(v) if v is not None else None,) for v in flat_values]
    ...
    log.info(
        ...
        f"{len(flat_values)} key(s) via {tmp_name} (key type: {col_type})..."
    )
    try:
        cur.execute(f"CREATE TABLE {tmp_name} (k0 {col_type})")
        cur.executemany(f"INSERT INTO {tmp_name} (k0) VALUES (?)", insert_rows)
        ...
        # The destination connection is reused across every flow for the
        # same server+database (ConnectionPool caches one connection per
        # server+database pair), so an orphaned temp table would still be
        # sitting around for the next flow that reuses this connection.
        # Always try to drop it, even if CREATE/INSERT/INDEX failed partway,
        # and verify + log the outcome rather than assuming it worked.
        try:
            cur.execute(f"IF OBJECT_ID('tempdb..{tmp_name}') IS NOT NULL DROP TABLE {tmp_name}")
            conn.commit()
            cur.execute(f"SELECT OBJECT_ID('tempdb..{tmp_name}')")
            still_exists = cur.fetchone()[0] is not None
            if still_exists:
                log.warning(f"[client={client_id}] Temp table {tmp_name} still exists after DROP attempt")
            else:
                log.info(f"[client={client_id}] Temp table {tmp_name} dropped successfully")
        except Exception as cleanup_err:
            log.warning(f"[client={client_id}] Failed to drop temp table {tmp_name}: {cleanup_err}")
    ...
    def _load_side(conn, side, sql, label):
        ...
        if log_preload:
            def progress_cb(loaded, lbl=label):
                ...
                audit.flush()
        return fetch_custom_query_side(
            conn, side, sql, ref_source_cols if side == "source" else ref_dest_cols,
            client_id, runtime_cfg, label=label, progress_cb=progress_cb,
        )

    try:
        source_rows_cache = _load_side(src_conn, "source", source_query, "source")
        destination_rows_cache = fetch_destination_filtered_by_keys(
            dst_conn, destination_query, ref_dest_cols, list(source_rows_cache.keys()),
            client_id, flow_id, runtime_cfg, cache_label_suffix="::validate",
        )
    except Exception as e:
...
def _load_custom_query_side(pool, audit, mapping, catalog, runtime_cfg, side, ref_cols, label, source_keys=None):
    """Run source/destination fetch query and return {join_key: row_dict}.

    FIX (see chat): destination used to always run the full, unfiltered
    destination_fetch_query -- pulling the entire destination table (20M+
    rows) even though we only ever need the rows matching source's keys.
    Now that source is loaded first at the call site below, its keys are
    passed in here as source_keys and used to scope the destination fetch
    via fetch_destination_filtered_by_keys (temp table + indexed JOIN),
    the same fix already applied on the validate path. Source is always
    fetched in full, unfiltered, since we need everything it has."""
    mapping = resolve_mapping_from_client(mapping, catalog)
    client_id = str(mapping.get("client_id", ""))
    flow_id = str(mapping.get("flow_id", ""))
    fetch_cfg = resolve_fetch_config(mapping, catalog)
    ...
    if side == "destination":
        rows = fetch_destination_filtered_by_keys(
            conn, sql, ref_cols, source_keys or [], client_id, flow_id, runtime_cfg,
            cache_label_suffix="::insert",
        )
    else:
        rows = fetch_custom_query_side(conn, "source", sql, ref_cols, client_id, runtime_cfg, label="source")
...
def _insert_custom_query_batch(pool, audit, mappings, catalog, runtime_cfg):
    ...
    try:
        ...
        destination_rows_cache, _ = _load_custom_query_side(
            ...
            source_keys=list(source_rows_cache.keys()),
        )
        dst_conn = pool.get(resolved["destination_server"], resolved["destination_database"])
    except Exception as e:
        audit.log(client_id=client_id, flow_id=flow_id, issue_type="CONNECTION_ERROR",
